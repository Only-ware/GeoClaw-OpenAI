...
1. Web 端自适应布局优化
   - 聊天区域改为随浏览器窗口高度动态调整（`100dvh` 视口逻辑）。
   - 会话列表与消息区在桌面/移动端均保持可滚动，不会因窗口变化挤压失效。

2. 最新消息自动跟随优化
   - 新消息到达时默认滚动到最下方（最新消息）。
   - 当用户手动上滚浏览历史时保留当前位置；回到底部后继续自动跟随。
   - 浏览器窗口大小变化时，若当前在底部会自动保持在最新消息位置。

3. 版本与文档同步
   - 包版本与运行时版本升级为 `3.2.3`。
   - README、CHANGELOG 同步更新，明确本次 Web 体验增强点。
