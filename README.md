...
在 v3.1.2 中，NL 规划会读取 `soul.md/user.md`，并按“研究区域优先”执行路由；当用户显式给出 `city/bbox/data-dir` 时优先保持原始输入范围。  
...
- 单元测试：`python3 -m unittest discover -s src/geoclaw_qgis/tests`（100/100 通过）
