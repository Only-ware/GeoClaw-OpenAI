...
- 团队一页安装手册（快速部署/升级/重装）：[`docs/team-install-onepager.md`](docs/team-install-onepager.md)
...
0. Web 端体验优化（新增）
- 对话区高度随浏览器窗口变化自动适配（桌面/移动端）。
- 新消息自动滚动到最底部；用户手动上滚查看历史时保留当前位置。
- 当窗口尺寸变化且当前在底部时，自动保持在最新消息。

0.1 自更新自动分支识别（新增）
- `geoclaw-openai update` 默认自动识别当前仓库的上游分支（如 `origin/master`）。
- 若未配置上游分支，会回退到当前本地分支；仍不可用时再回退 `main/master`。
- 常见场景无需再手工追加 `--branch master`。
...
```bash
geoclaw-openai update --remote origin --branch master
```
...
在 v3.2.4 中，NL 规划会读取 `soul.md/user.md`，并按“研究区域优先”执行路由；当用户显式给出 `city/bbox/data-dir` 时优先保持原始输入范围。  
