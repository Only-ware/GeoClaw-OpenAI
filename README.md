...
- 修复显式城市输入的错误路由风险，显式 `--city/--bbox/--data-dir` 优先保持原始研究区域。
...
在 v3.1.1 中，NL 规划会读取 `soul.md/user.md`，并在商场选址等场景触发注册 Skill 优先路由。  
...
- 单元测试：`python3 -m unittest discover -s src/geoclaw_qgis/tests`（93/93 通过）
